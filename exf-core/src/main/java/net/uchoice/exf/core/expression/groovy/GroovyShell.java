    ...
    public GroovyShell(ClassLoader parent, Binding binding, final CompilerConfiguration config) {
		...
		this.loader = AccessController.doPrivileged(new PrivilegedAction<GroovyClassLoader>() {
			public GroovyClassLoader run() {
				// 支持自定义注入GroovyClassLoader类加载器
				return GroovyClassLoader.class.isAssignableFrom(parentLoader.getClass())
						? (GroovyClassLoader) parentLoader : new GroovyClassLoader(parentLoader, config);
			}
		});
