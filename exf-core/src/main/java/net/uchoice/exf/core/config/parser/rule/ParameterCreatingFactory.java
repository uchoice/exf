...
public class ParameterCreatingFactory extends AbstractObjectCreationFactory<Variable> {
    ...
    public Variable createObject(Attributes attributes) throws Exception {
        ...
        if (StringUtils.isBlank(name) || StringUtils.isBlank(value)) {
            throw new VariableConfigException(
                ErrorMessage.code("S-EXF-01-01-008", name, VariableMode.IN.name(), value));
        }
        VariableSpec variableSpec = new VariableSpec();
        variableSpec.setMode(VariableMode.IN);
